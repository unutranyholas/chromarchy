...
describe("plugin package", () => {
  it("runs the clean release payload and deterministically rebuilds its bundle", async () => {
    ...
    const bundle = path.join(root, "plugin/dist/chromarchy.mjs");
    ...
    try {
      ...
      expect(files).toContain("plugin/dist/chromarchy.mjs");
      ...
      expect(fs.readdirSync(path.join(copy, "plugin/dist"))).toEqual(["chromarchy.mjs"]);
      ...
      expect(paletteQml).toContain('command: [root.helper, "worker"]');
      expect(paletteQml).toContain("stdinEnabled: true");
      ...
      const generated = run(wrapper, ["generate", "--config-json", JSON.stringify(TEST_CONFIG)], {
        cwd: copy,
      });
      expect(generated.status).toBe(0);
      expect(JSON.parse(generated.stdout).config).toEqual(TEST_CONFIG);
      expect(
        run(wrapper, ["generate", "--config-json", '{"mode":"dark"}'], {
          cwd: copy,
        }).status,
      ).toBe(1);
...
`,
      ...
      expect(
        run(wrapper, ["apply", "--config-json", JSON.stringify(TEST_CONFIG)], {
          cwd: copy,
          env,
        }).status,
      ).toBe(0);
      ...
      const first = spawn(wrapper, ["apply", "--config-json", JSON.stringify(TEST_CONFIG)], {
        cwd: copy,
        env: concurrentEnv,
      ...
      const recipe = path.join(home, ".local/state/omarchy/chromarchy/recipes/chromarchy.json");
      const undo = path.join(home, ".local/state/omarchy/chromarchy/undo.json");
      const before = {
        theme: fs.readFileSync(themeName, "utf8"),
        recipe: fs.existsSync(recipe) ? fs.readFileSync(recipe, "utf8") : null,
        undo: fs.existsSync(undo) ? fs.readFileSync(undo, "utf8") : null,
      };
      const second = run(wrapper, ["apply", "--config-json", JSON.stringify(TEST_CONFIG)], {
        cwd: copy,
        env: concurrentEnv,
      });
